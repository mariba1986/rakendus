<?php
  ...
  if(isset($_POST["submitUserData"])){
	...
	if ($notice == "ok") {
		...
		$notice = "Uue kasutaja loomisel tekkis tehniline tõrge: " .$notice;
	}
	}//kui kõik korras
  ...
  <body>
	...
	<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	  ...
	  <?php
		...
		for ($i = date("Y") - 15; $i >= date("Y") - 110; $i --){  //võetud tänasest 15 aastat tagasi, ehk et kõige noorem lehe kasutaja võiks olla 15.
			echo "\t \t" .'<option value="' .$i .'"';
			if ($i == $birthYear){
				echo " selected ";
			}
